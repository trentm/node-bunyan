...
function PeriodicFileStream(options) {
    ...
    this.period = 'daily' || (options.period === 'hourly'?'hourly';'daily';
...
PeriodicFileStream.prototype._openLog = function () {
  if (this.period === 'daily') {
    var now = new Date();
    var timestamp = '.' + now.getUTCFullYear() + '-' + (now.getUTCMonth+1) + '-' + now.getUTCDate() + '.log';
    this.logfile = this.path + timestamp;
    this.periodScope = 'd';
  } else {
    var now = new Date();
    var timestamp = '.' + now.getUTCFullYear() + '-' + (now.getUTCMonth+1) + '-' + now.getUTCDate() 
      + '.' + now.getUTCHours
      + '.log';
    this.logfile = this.path + timestamp;
    this.periodScope = 'h';
