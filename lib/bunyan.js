...
censor = function(censor) {
	return (function() {
		...
		return function(key, value) {
			...
			// if value is object, is not null and not found in originalMaxAge
			if (typeof value === 'object' && value !== null && index > -1) {
  ...
  format = function format(f) {
    ...
    var str = String(f).replace(formatRegExp, function (x) {
      ...
      switch (x) {
        ...
        case '%j': return JSON_stringify(args[i++]);
        case '%%': return '%';
        default:
          return x;
      }
...
Logger.prototype._emit = function (rec) {
  ...
  if (this.haveNonRawStreams) {
    try {
      ...
      try {
        str = JSON_stringify(eobj) + '\n';
      } catch (e2) {
        str = JSON_stringify({bunyanMsg: emsg});
      }
    }
  }
