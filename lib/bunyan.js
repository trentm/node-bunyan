...
Logger.prototype.addStream = function addStream(s, defaultLevel) {
    ...
    case 'periodic-file':
        assert.ok(!s.stream,
                  '"periodic-file" stream should not give a "stream"');
        assert.ok(s.path);
        assert.ok(mv, '"periodic-file" stream type is not supported: '
                      + 'missing "mv" module');
        s.stream = new PeriodicFileStream(s);
        if (!s.closeOnExit) {
            s.closeOnExit = true;
        }
        break;
...
Logger.prototype.reopenFileStreams = function () {
    ...
    self.streams.forEach(function (s) {
        if (s.type === 'file') {
            ...
            s.stream = fs.createWriteStream(s.logfile || s.path,
                {flags: 'a', encoding: 'utf8'});
            s.stream.on('error', function (err) {
                self.emit('error', err, s);
            });
...
function PeriodicFileStream(options) {

    if (!options) {
      throw new Error(format('null options value'));
    }
    if (options.path) {
      this.path = options.path;
    } else {
      throw new Error(format('empty path parameter'));
    }

    this.period = (options.period === 'hourly')?'hourly':'daily';
...
util.inherits(PeriodicFileStream, EventEmitter);

PeriodicFileStream.prototype._openLog = function () {
  var _DEBUG = false;
  if (_DEBUG)
    console.log('-- _openLog:');
  ...
  if (this.period === 'daily') {
    ...
    now.getFullYear() + '-' +
      ...
      ("0" + now.getDate()).slice(-2) + '.log';

    this.logfile = this.path + timestamp;
    this.periodScope = 'd';
  } else {
  ...
  if (_DEBUG)
    console.log ('-- _openLog: opening log file ' + this.logfile);

  this.stream = fs.createWriteStream(this.logfile, {flags: 'a', encoding: 'utf8'});

  if (this.rotQueue) {
    while(this.rotQueue.length > 0) {
      this.stream.write(this.rotQueue.pop());
    }
  }
...
PeriodicFileStream.prototype._setupNextRot = function () {
    var _DEBUG = false;
    if (_DEBUG)
        console.log('-- _setupNextRot: ');
    ...
    if (_DEBUG)
        console.log('-- delay time is : %s', delay);
    ...
    if (_DEBUG)
      console.log('-- _setupNextRot: calling rotate');
...
PeriodicFileStream.prototype._nextRotTime = function _nextRotTime(first) {
    var _DEBUG = false;
    if (_DEBUG)
        console.log('-- _nextRotTime: %s', this.periodScope);
    var d = new Date();
...
PeriodicFileStream.prototype.rotate = function rotate() {
    // XXX What about shutdown?
    var self = this;
    var _DEBUG = false;

    if (_DEBUG) {
        console.log('-- [%s, pid=%s] rotating %s at %s',
            new Date(), process.pid, self.path, self.rotAt);
    }
...
PeriodicFileStream.prototype.write = function write(s) {
    if (this.rotating) {
        this.rotQueue.push(s);
        return false;
    } else {
...
PeriodicFileStream.prototype.end = function end(s) {
    this.stream.end();
};

PeriodicFileStream.prototype.destroy = function destroy(s) {
    this.stream.destroy();
};

PeriodicFileStream.prototype.destroySoon = function destroySoon(s) {
    this.stream.destroySoon();
};
...
module.exports.PeriodicFileStream = PeriodicFileStream;
