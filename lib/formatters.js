 ...
 *      - mode {String} Formatting mode. Defaults to 'wide'
 ...
 *      - Warning: The returned formatter functions might *mutate* the given
 *        record. If you need to given `rec` argument unchanged, you must
 *        pass in a copy via `formatters.recCopy(rec)`.
 *      - Warning: The formatter functions can only take valid Bunyan record
 *        objects, else they may blow up.
 *
 * TODO: clarify usage of isValidRecord, etc.
 */
function createFormatter(options) {
    if (!options) options = {};

    var mode = options.mode || 'wide';  // actually default might be 'paul'
    if (mode === 'paul')  // backward compat name
        mode = 'wide';
    ...
    case 'wide':
         ...
         */
        return function wideFormatter(rec) {
            //warn("XXX wideFormatter: rec=%j", rec)
            delete rec.v;
             ...
             */
            var time = stylize('[' + rec.time + ']', 'time');
            delete rec.time;
            ...
            if (rec.res) {
                ...
                var s = formatRes(res);
                if (s)
                    details.push(indent(formatRes(res)));
    ...
    case 'short':
        return function shortFormatter(rec) {
            delete rec.v;
             ...
             * case we can safely chop off the date information.
             */
            var time = (rec.time[10] === 'T' ? rec.time.substr(11) : rec.time);
    ...
    default:
        throw new Error(_('unknown formatter mode: %j', mode));
    }
}
 ...
 *
 * Note: This guards against Bunyan formatter mutation of the given 'rec'.
 *
 * @param rec {Bunyan record object}
...
function format(rec, options) {
    return createFormatter(options)(recCopy(rec));
}
