...
//var Logger = require('/home/local/DARKSIDE/kris.re/bunyan-screenlogger/logger.js');
...
function printHelp() {
    ...
    p('                Output to a custom formatter. Value is passed to require, so can');
    p('                be either a module name or a path');
...
function gotRecord(file, line, rec, opts)
{
...
function emitNextRecord(opts)
{
    ...
    for (;;) {
        ...
        emitRecord(rec.rec, rec.line, opts);
    }
}
...
function parseArgv(argv) {
    ...
    var newArgs = [];
    var optTakesArg = {'f': true, 'd': true, 'o': true, 'c': true, 'l': true, 'p': true};
...
function handleLogLine(file, line, opts) {
    ...
    if (file === null)
        return emitRecord(rec, line, opts);

    return gotRecord(file, line, rec, opts);
}
 ...
 */
function processStdin(opts, callback) {
    readingStdin = true;
    ...
    stdin.on('data', function (chunk) {
        ...
        if (length > 1) {
            handleLogLine(null, leftover + lines[0], opts);
        }
        leftover = lines.pop();
        length -= 1;
        for (var i = 1; i < length; i++) {
            handleLogLine(null, lines[i], opts);
        }
    });
    stdin.on('end', function () {
        if (leftover) {
            handleLogLine(null, leftover, opts);
            leftover = '';
        }
        callback();
    });
 ...
 */
function processPids(opts, callback) {
    var leftover = '';  // Left-over partial line from last chunk.
 ...
 * @param callback {Function} `function ()`
 */
function processFile(file, opts, callback) {
    ...
    stream.on('data', function (data) {
        ...
        if (length > 1) {
            handleLogLine(file, leftover + lines[0], opts);
        }
        leftover = lines.pop();
        length -= 1;
        for (var i = 1; i < length; i++) {
            handleLogLine(file, lines[i], opts);
        }
    });
    ...
    stream.on('end', function () {
        streams[file].done = true;
        if (leftover) {
            handleLogLine(file, leftover, opts);
            leftover = '';
        } else {
            emitNextRecord(opts);
        }
...
function main(argv) {
    ...
    var retval = 0;
    if (opts.pids) {
        processPids(opts, function (code) {
            cleanupAndExit(code);
        });
        ...
        asyncForEach(files,
            function (file, next) {
                processFile(file, opts, function (err) {
                    if (err) {
        ...
        processStdin(opts, function () {
            cleanupAndExit(retval);
        });
    }

    if (opts.Formatter) {
        logger = new opts.Formatter(opts);
    } else {
        logger = new Formatter(opts);
    }
